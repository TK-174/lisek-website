...
4. **Usługi i cennik** (`#uslugi`, `ServicesPricing.astro`) — single combined section that on 2026-06-11 replaced the former separate Usługi/Cennik/Oferta sections (user request: offers and service descriptions should sit next to prices). Contains: (a) 5 service cards from `salon.ts`; (b) `#cennik` sub-block (`role="region"`) with the `PricingTabs.tsx` island — full WAI-ARIA tab pattern (tablist/tab/tabpanel, aria-selected, aria-controls, roving tabindex, ←/→ wrap, Home/End, Space/Enter), panel headings are `h4`; (c) `#promocje` sub-block (`role="region"`) with two compact promo cards — adoption −50% (Dalabuszki shelter, "wspomnij o adopcji") and Karta stałego klienta (6 stamps → −50%) — each showing −50% as a price tag next to its description. Anchors `#uslugi` and `#cennik` preserved; header/footer nav has 5 links ("Usługi i cennik" replaces separate Usługi + Cennik). 2026-06-11: renamed "Oferta"/"Oferty specjalne" → **"Promocje"** per user (user typed "Pomocje", corrected to proper Polish "Promocje"); anchor `#oferta` → `#promocje` (old `#oferta` deep links no longer resolve).
...
- 2026-06-11: **Hero stats strip removed** (Doświadczenie 10+ lat / Usługi / Pupile) — user: the "10+ lat" experience claim is not true; salon credentials are the certificates, not years. Do not re-add experience claims.
