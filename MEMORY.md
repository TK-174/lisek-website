...
4. **Usługi i cennik** (`#uslugi`, `ServicesPricing.astro`) — single combined section that on 2026-06-11 replaced the former separate Usługi/Cennik/Oferta sections (user request: offers and service descriptions should sit next to prices). Contains: (a) 5 service cards from `salon.ts`; (b) `#cennik` sub-block (`role="region"`) with the `PricingTabs.tsx` island — full WAI-ARIA tab pattern (tablist/tab/tabpanel, aria-selected, aria-controls, roving tabindex, ←/→ wrap, Home/End, Space/Enter), panel headings are `h4`; (c) `#oferta` sub-block (`role="region"`) with two compact promo cards — adoption −50% (Dalabuszki shelter, "wspomnij o adopcji") and Karta stałego klienta (6 stamps → −50%) — each showing −50% as a price tag next to its description. All legacy anchors (`#uslugi`, `#cennik`, `#oferta`) preserved; header/footer nav reduced to 5 links ("Usługi i cennik" replaces separate Usługi + Cennik).
